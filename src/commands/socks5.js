...
exports.run = async(client, message, args) => {
    ...
    got('https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&timeout=5000').then(response => {

        fs.writeFile('./src/storage/socks5.txt', response.body, function (err) {
            if (err) return console.log(err);
            console.log('\n\x1b[32m%s\x1b[0m', `$[INFO]: Wrote data to file socks5.txt`);
          });
